...
// import gud from "gud";
import { Languages, availableLanguages } from "../languages/index";

export default class Calendar extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      monthNames: [...Languages.PT.MONTHS],
      days: [...Languages.PT.DAYS],
      today: moment(),
  ...
  renderMultiEventBlock(eventsEachDay, startDate, length, props, arrowLeft, arrowRight) { 
    ...
    for (let i = 0; i < length; i++) {
      ...
      while (eventsEachDay[startDate - 1 + i].length <= chosenRow) {
        eventsEachDay[startDate - 1 + i].push(<div className="event below placeholder" key={`placeholder-`}></div>);
      }

      //rest of event that is under the main banner
      eventsEachDay[startDate - 1 + i][chosenRow] = <div className="isEvent event below" key={`filler-`}></div>;
    }
  
    //render event
    eventsEachDay[startDate - 1][chosenRow] = <div className="isEvent" key={`multi-event-${chosenRow}`}><MultiEvent {...props} {...multiEventProps} length={length} arrowLeft={arrowLeft} arrowRight={arrowRight} key={`multi-event-`}/></div>;
  }
  ...
  renderSingleEvent(eventsEachDay, date, props) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      if (nodes[i].props.className.includes("event") && !nodes[i].props.className.includes("isEvent")) { //target only placeholders
        nodes[i] = <div className="isEvent" key={`single-event-`}><Event {...props} key={`single-event-`}/></div>;
        foundEmpty = true;
        break;
      }
    }
    if (!foundEmpty) {
      eventsEachDay[date - 1].push(<div className="isEvent" key={`single-event-`}><Event {...props} key={`single-event-`}/></div>)
    }
  }
        ...
        { this.state.showFooter && 
          <div className="calendar-footer">
            ...
            <div css={css`
              ...
              margin-right: 3px;
            `}>
              <a href={"https://calendar.google.com/calendar/r?cid=" + this.props.calendarId} target="_blank" rel="noreferrer" css={css`
                font-size: 14px;
